...
import static io.cetrax.jdbc.JdbcUtils.addParameters;
import static io.cetrax.jdbc.JdbcUtils.quote;

abstract class AbstractBaseCetraxOnJdbc {

    // internal database used to keep track of namespace lookup tables
    private static final String cetraxInternalDatabaseName = "cetrax";
    ...
    protected AbstractBaseCetraxOnJdbc(final DataSource dataSource) {
        this.dataSource = dataSource;
    }

    protected String getCetraxInternalDatabaseName() {
        return cetraxInternalDatabaseName;
    }
    ...
    protected void createNamespace(final String namespace) throws IOException {
        ...
        try {
            ...
            // create cetrax internal database if not exists
            ...
            final String sql = String.format("INSERT INTO %s.%s SET %s = ?, %s = ? ON DUPLICATE KEY UPDATE %s = ?",
                    quote(cetraxInternalDatabaseName), quote(getNamespaceLookupTableName()),
    ...
    protected Collection<String> getNamespaces() throws IOException {
        ...
        final String sql = String.format("SELECT %s FROM %s.%s",
                quote(getNamespaceColumnName()),
                quote(cetraxInternalDatabaseName),
    ...
    protected String getDatabaseNameForNamespace(final String namespace) {
        ...
        return String.format("cetrax-%s-%s",
    ...
    protected String getCreateInternalDatabaseSql() {
        return String.format("CREATE DATABASE IF NOT EXISTS %s", quote(cetraxInternalDatabaseName));
    }
